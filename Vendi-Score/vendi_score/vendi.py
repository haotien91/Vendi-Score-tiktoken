...
# charless
def score_dual_sparse(X, q=1, normalize=True, k=100):
    if normalize:
        X = preprocessing.normalize(X, axis=1, norm='l2', copy=False)
    
    n, d = X.shape
    X_sparse = sparse.csr_matrix(X)
    
    # 使用 Lanczos 算法計算前 k 個最大特徵值
    S = X_sparse.T @ X_sparse
    w, _ = scipy.sparse.linalg.eigsh(S / n, k=min(k, n-1), which='LM')
